...
module BDDAPRON_DOMAIN(Param: BDDAPRON_PARAM) =
  (struct
    ...
    let top env = Bdddomain1.top man env
    let bot env = Bdddomain1.bottom man env
    ...
    let eq a b = Bdddomain1.is_eq man a b

    let subseteq a b = Bdddomain1.is_leq man a b

    let print fmt a = Bdddomain1.print fmt a
    ...
    let join a b = Bdddomain1.join man a b

    let meet a b = Bdddomain1.meet man a b
        
    let widen a b = Bdddomain1.widening man a b

    let widen_threshold a b threshold = Bdddomain1.widening_threshold man a b threshold

    let assign dom var a = Bdddomain1.assign_lexpr ~relational:true man cond dom [var] [a] None, ()
    ...
    let filter dom b = 
      Bdddomain1.meet_condition man cond dom b, Bdddomain1.meet_condition man cond dom (Expr1.Bool.dnot cond b), ()
    ...
    let add_vars ?(init=true) (*?(init=true) on suppose que y'a jamais besoin d'initialiser*) dom vars =
      ...
      let dom = Bdddomain1.change_environment man dom env in
      if (init) then
        (
